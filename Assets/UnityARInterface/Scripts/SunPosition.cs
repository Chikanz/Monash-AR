...
//http://guideving.blogspot.com.au/2010/08/sun-position-in-c.html
public class SunPosition : MonoBehaviour
{
    ...
    {
        if(!timeOverride)
            CalculateSunPosition(DateTime.Now, -37.8765, 145.0442);
        else
            CalculateSunPosition(new DateTime(2017,12,18,hour,0,0), -37.8765, 145.0442);
    }

    public void Update()
    {
        if(hour != pastHour && timeOverride)
        {
            CalculateSunPosition(new DateTime(2017, 12, 18, hour, 0, 0), -37.8765, 145.0442);
            pastHour = hour;
        }
     ...
     */
    private void CalculateSunPosition(DateTime dateTime, double latitude, double longitude)
    {
        // Convert to UTC  
        ...
        // Altitude  
        Debug.Log("Altitude: " + altitude * Rad2Deg); //x

        // Azimut  
        Debug.Log("Azimuth: " + azimuth * Rad2Deg); //y

        //Set light rotation based on calculations
        //https://en.wikipedia.org/wiki/Horizontal_coordinate_system
        transform.rotation = Quaternion.Euler((float)(altitude * Rad2Deg), (float)(azimuth * Rad2Deg), 0);
    }
